...
public class DateTimeUtil {
	...
	/**
	 * 获取系统当前时间并格式化为yyyy即类似2014格式
	 * 
	 * @param 无
	 * @return 系统当前时间并格式化为yyyy即类似2014格式
	 */
	public static int getYaer() {
		Calendar sdf=Calendar.getInstance();
		return sdf.get(Calendar.YEAR);
	}
	
	/**
	 * 获取系统当前日期并格式化为yyyyMM即类似201408格式
	 * 
	 * @param 无
	 * @return 系统当前日期并格式化为yyyyMM即类似201408格式
	 */
	public static String getYaerAndMonth() {
		SimpleDateFormat sdf = new SimpleDateFormat("yyyyMM");
		return sdf.format(new Date());
	}
	
	/**
	 * 获取系统当前日期并格式化为yyyyMMdd即类似20140810格式
	 * 
	 * @param 无
	 * @return 系统当前日期并格式化为yyyyMMdd即类似20140810格式
	 */
	 ...
	 * 获取系统当前日期之前或者之后某天的日期
	 * 
	 * @someday 当前时间往前推或者往后推的天数,+ after,- before
	 * @return 系统当前时间并格式化为yyyyMMdd即类似20150708格式
	 */	
    public static String getSomeDay(int someday) {       
